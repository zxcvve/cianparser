...
from cianparser.constants import (
    SPECIFIC_FIELDS_FOR_RENT_LONG,
    SPECIFIC_FIELDS_FOR_RENT_SHORT,
...
class BaseListPageParser:
    def __init__(
        ...
        self.start_page = (
            1
            if (
                additional_settings is None
                or "start_page" not in additional_settings.keys()
            )
            else additional_settings["start_page"]
        )
        self.end_page = (
            100
            if (
                additional_settings is None
                or "end_page" not in additional_settings.keys()
            )
            else additional_settings["end_page"]
        )
    ...
    def define_average_price(self, price_data):
        if "price" in price_data:
            self.average_price = (
                self.average_price * self.count_parsed_offers + price_data["price"]
            ) / self.count_parsed_offers
        elif "price_per_month" in price_data:
            self.average_price = (
                self.average_price * self.count_parsed_offers
                + price_data["price_per_month"]
            ) / self.count_parsed_offers
